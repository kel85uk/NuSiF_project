...
GrayScaleImage::GrayScaleImage(Array<unsigned char>& A_)
{
   size_[0] = A_.getSize(0)-2;
   size_[1] = A_.getSize(1)-2;
   image_.resize( size_[0] * size_[1]);
   for (int i=0; i<size_[0]; i++) 
       for (int j=0; j<size_[1]; j++)
           image_[(size_[1]-j-1)*size_[0] + i] = A_(i+1,j+1);
}
...
unsigned char GrayScaleImage::operator() ( int x, int y ) const
{
